...
// -----------------------------------------------------------------------
// Nomi degli allegati: il mittente controlla questo campo, quindi non deve
// mai poter scegliere il percorso locale usato durante l'esportazione.
// -----------------------------------------------------------------------

{
  check("allegato: toglie traversal Unix", safeAttachmentName("../../config.json") === "config.json");
  check("allegato: toglie traversal Windows", safeAttachmentName("..\\..\\secret.txt") === "secret.txt");
  check("allegato: punto diventa fallback", safeAttachmentName("..") === "allegato");
  check("allegato: separatori e controlli", safeAttachmentName("a:b\u0000.txt") === "a_b_.txt");
  check("allegato: limita a 180 byte UTF-8", utf8Encode(safeAttachmentName("🙂".repeat(100))).length === 180);
}
