...
use keyboard_cipher_core::api::{IncomingFile, IncomingItem, SenderStatus};
...
/// Layout: sender(32) | sender_status (come in `encode_incoming_item`) |
/// nostro(1) | nome (str_u16) | mime (str_u16) | sent_at_unix i64LE(8) |
/// contenuto_len u32LE(4) + contenuto.
pub fn encode_incoming_file(f: &IncomingFile) -> Vec<u8> {
    let mut w = Writer::new();
    w.bytes(f.sender.as_bytes());
    match &f.sender_status {
        SenderStatus::New => {
            w.u8(0);
        }
        SenderStatus::Known { label, verified } => {
    ...
    w.u8(u8::from(f.nostro));
    w.str_u16(&f.file.meta.name);
    w.str_u16(&f.file.meta.mime);
    w.i64_le(f.file.sent_at_unix);
    let content = f.file.content.as_slice();
    w.u32_le(u32::try_from(content.len()).unwrap_or(u32::MAX));
    w.bytes(content);
    w.into_vec()
}
