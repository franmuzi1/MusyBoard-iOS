...
use keyboard_cipher_core::backup;
...
/// La propria chiave pubblica (32 byte). Serve dopo [`mb_import_backup`]: un
/// backup porta solo il segreto, e senza questa non c'e' modo di recuperare
/// la pubblica per rimetterla in `config.json` senza ricalcolarla altrove.
#[no_mangle]
pub extern "C" fn mb_my_public() -> u64 {
    match with_session(|s| *s.identity().public().as_bytes()) {
        Some(bytes) => marshal::pack_ok(bytes.to_vec()),
        None => marshal::pack_err(marshal::ERR_NO_SESSION),
    }
}
...
// ---------------------------------------------------------------------------
// Backup cifrato con passphrase (Argon2id, `keyboard_cipher_core::backup`).
//
// Il portachiavi e' byte OPACHI per `backup::export`/`import` — la sua
// serializzazione non e' affar loro. Qui non se ne inventa una nuova: JS
// costruisce il blob con lo stesso codec gia' usato per `config.json`
// (`encodePeerRecordJs`/`encodePrekeyRecordJs`, con un prefisso di lunghezza
// per record), lo passa in ingresso a `mb_export_backup` e lo riceve indietro
// da `mb_import_backup` — questo crate lo tratta come byte, esattamente come
// fa il core.
// ---------------------------------------------------------------------------

/// Cifra identita' + portachiavi (byte opachi, forniti da JS) con una
/// passphrase. Consuma RNG (salt e nonce).
#[no_mangle]
pub extern "C" fn mb_export_backup(
    pass_ptr: u32,
    pass_len: u32,
    keyring_ptr: u32,
    keyring_len: u32,
) -> u64 {
    // SAFETY: il chiamante garantisce le lunghezze dichiarate ai puntatori.
    let passphrase = unsafe { marshal::read_slice(pass_ptr, pass_len) };
    // SAFETY: idem.
    let keyring_bytes = unsafe { marshal::read_slice(keyring_ptr, keyring_len) };
    let mut rng = match rng::take() {
        Some(r) => r,
        None => return marshal::pack_err(marshal::ERR_RNG_NOT_SEEDED),
    };
    match with_session(|s| backup::export(s.identity(), keyring_bytes, passphrase, &mut rng)) {
        Some(Ok(blob)) => marshal::pack_ok(blob),
        Some(Err(e)) => marshal::pack_core_err(e),
        None => marshal::pack_err(marshal::ERR_NO_SESSION),
    }
}

/// Apre un backup. Ritorna 32 byte di segreto + il portachiavi opaco
/// (concatenati) — non installa nessuna sessione da solo: il chiamante
/// prosegue con `mb_load_identity` + N × `mb_restore_peer`/
/// `mb_restore_prekey_record` (decodificando il portachiavi con lo stesso
/// codec con cui lo ha costruito) + `mb_finish_load`, esattamente come per
/// l'idratazione da `config.json`. Non richiede una sessione attiva.
#[no_mangle]
pub extern "C" fn mb_import_backup(pass_ptr: u32, pass_len: u32, blob_ptr: u32, blob_len: u32) -> u64 {
    // SAFETY: il chiamante garantisce le lunghezze dichiarate ai puntatori.
    let passphrase = unsafe { marshal::read_slice(pass_ptr, pass_len) };
    // SAFETY: idem.
    let blob = unsafe { marshal::read_slice(blob_ptr, blob_len) };
    match backup::import(blob, passphrase) {
        Ok(opened) => {
            let mut out = Vec::with_capacity(32usize.saturating_add(opened.keyring.len()));
            out.extend_from_slice(opened.secret.as_ref());
            out.extend_from_slice(&opened.keyring);
            marshal::pack_ok(out)
        }
        Err(e) => marshal::pack_core_err(e),
    }
}

// ---------------------------------------------------------------------------
// Allegati (`keyboard_cipher_core::file`). A differenza dei messaggi di
// testo, che qui cifrano "a epoca" (decisione J, `effimero = false`), gli
// allegati usano SEMPRE la catena di forward secrecy (decisione I) — e' la
// raccomandazione esplicita del core stesso ("un allegato senza catena e' un
// buco piu' grosso di un messaggio senza"), non una scelta di questo crate.
// Sono comunque la stessa conversazione a due: catena condivisa con i
// messaggi, non uno stato a parte.
// ---------------------------------------------------------------------------

#[no_mangle]
pub extern "C" fn mb_encrypt_file(
    peer_ptr: u32,
    name_ptr: u32,
    name_len: u32,
    mime_ptr: u32,
    ...
    // SAFETY: il chiamante garantisce 32 byte validi a `peer_ptr`.
    let peer = PublicKey::from_bytes(*unsafe { marshal::read_key32(peer_ptr) });
    // SAFETY: il chiamante garantisce `name_len` byte validi a `name_ptr`.
    let name = match unsafe { marshal::read_str(name_ptr, name_len) } {
        Some(s) => s.to_owned(),
        None => return marshal::pack_err(marshal::ERR_INVALID_UTF8),
    };
    // SAFETY: il chiamante garantisce `mime_len` byte validi a `mime_ptr`.
    let mime = match unsafe { marshal::read_str(mime_ptr, mime_len) } {
        Some(s) => s.to_owned(),
        None => return marshal::pack_err(marshal::ERR_INVALID_UTF8),
    };
    // SAFETY: il chiamante garantisce `content_len` byte validi a `content_ptr`.
    let content = unsafe { marshal::read_slice(content_ptr, content_len) };
    let meta = keyboard_cipher_core::file::FileMeta { name, mime };
    let mut rng = match rng::take() {
        Some(r) => r,
        None => return marshal::pack_err(marshal::ERR_RNG_NOT_SEEDED),
    };
    match with_session(|s| s.encrypt_file_with(&peer, &meta, content, now_unix, &mut rng, true)) {
        Some(Ok(blob)) => marshal::pack_ok(blob),
        Some(Err(e)) => marshal::pack_core_err(e),
        None => marshal::pack_err(marshal::ERR_NO_SESSION),
    }
...
#[no_mangle]
pub extern "C" fn mb_decrypt_file(bytes_ptr: u32, bytes_len: u32, now_unix: i64) -> u64 {
    // SAFETY: il chiamante garantisce `bytes_len` byte validi a `bytes_ptr`.
    let bytes = unsafe { marshal::read_slice(bytes_ptr, bytes_len) };
    match with_session(|s| s.handle_incoming_file(bytes, now_unix)) {
        Some(Ok(incoming)) => marshal::pack_ok(codec::encode_incoming_file(&incoming)),
        Some(Err(e)) => marshal::pack_core_err(e),
        None => marshal::pack_err(marshal::ERR_NO_SESSION),
    }
}
...
// ---------------------------------------------------------------------------
// Messaggio di gruppo (decisione K). Solo la cifratura ha bisogno di una
// funzione dedicata: un gruppo in arrivo passa dallo stesso
// `handle_incoming_text`/`mb_decrypt` di un messaggio a due — il core lo
// riconosce da solo (`ParsedBlob::Group`) e produce lo stesso
// `IncomingItem::Message`, con `gruppo=true` e `destinatari=N`, gia'
// codificato da `codec::encode_incoming_item`. Nessuna forward secrecy per
// decisione K1: e' l'unico posto che usa `encrypt_group`, non
// `encrypt_for_app_with`.
// ---------------------------------------------------------------------------

/// Cifra per piu' destinatari. `peers_ptr` punta a `peers_count * 32` byte
/// (chiavi concatenate, nessun separatore). Consuma RNG.
#[no_mangle]
pub extern "C" fn mb_encrypt_group(
    peers_ptr: u32,
    peers_count: u32,
    text_ptr: u32,
    text_len: u32,
    ...
    // SAFETY: il chiamante garantisce `peers_count * 32` byte validi a `peers_ptr`.
    let raw = unsafe { marshal::read_slice(peers_ptr, peers_count.saturating_mul(32)) };
    let mut destinatari = Vec::with_capacity(peers_count as usize);
    for chunk in raw.chunks_exact(32) {
        let Ok(arr): std::result::Result<[u8; 32], _> = chunk.try_into() else {
            return marshal::pack_err(marshal::ERR_BAD_INPUT);
        };
        destinatari.push(PublicKey::from_bytes(arr));
    }
    // SAFETY: il chiamante garantisce `text_len` byte validi a `text_ptr`.
    let plaintext = unsafe { marshal::read_slice(text_ptr, text_len) };
    let mut rng = match rng::take() {
    ...
    match with_session(|s| s.encrypt_group(&destinatari, plaintext, now_unix, &mut rng)) {
        Some(Ok(blob)) => marshal::pack_ok(blob.into_bytes()),
        Some(Err(e)) => marshal::pack_core_err(e),
        None => marshal::pack_err(marshal::ERR_NO_SESSION),
    }
